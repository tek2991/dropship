        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('admin.managers.update', ['manager' => $manager->id]) }}">
                        @method('PUT')
                        @csrf
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-input id="email" class="block mt-1 w-full" type="email" required
                                    value="{{ $manager->user->email }}" name="email" />
                            </div>
                            <div>
                                <x-label for="gender" :value="__('Gender')" />
                                <x-input-select id="gender" class="block mt-1 w-full" name="gender">
                                    <option value="male" {{ $manager->user->gender == 'male' ? 'selected' : '' }}>Male
                                    </option>
                                    <option value="female" {{ $manager->user->gender == 'female' ? 'selected' : '' }}>
                                        Female
                                    </option>
                                    <option value="" {{ $manager->user->gender == null ? 'selected' : '' }}>Not
                                        Applicable</option>
                                </x-input-select>
                            </div>
                            <div>
                    ...
                    <form method="POST" action="{{ route('admin.managers.update-password', ['manager' => $manager->id]) }}">
                        @method('PUT')
                        @csrf
